...
自动任务使用 RDS `automation_jobs` 和独立 ECS worker。每个阶段有 10 分钟租约，多个 worker 通过条件更新抢占任务。无待处理任务时只轮询数据库，不调用 Runtime。确认处于安全状态的竞价／交付失败最多自动重试两次；支付不确定时暂停且保留预留资金。租约到期但仍处于 `quoting` / `paying` / `delivering` 的任务需操作员核对，不能盲目复位。自动对账仍未实现。
...
- 自动任务队列、并发 worker 单次付款、租约恢复、付款不确定暂停、免重复付款的交付重试、旧任务授权兼容性。
...
- 专用演示账户 `demo@agentmarketplace.example` 使用独立 Cognito subject，通过 `ShowcaseUserSub` 和明确的任务／Agent ID 列表读取现有测试数据。该演示邮箱使用保留的 `.example` 域，不接收邮件；需要变更密码时由管理员处理。钱包使用权限由独立的 `PaymentDelegateSub` 明确授予，不能从只读共享推导支付权限，也不要复制已结算支付来填充展示数据。
