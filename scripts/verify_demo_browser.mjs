...
const verification = JSON.parse(
  await readFile(path.join(root, "artifacts/aws-demo-verification.json"), "utf8"),
);
...
try {
  for (const name of ["desktop", "mobile"]) {
    ...
    await expect(dialog.getByRole("button", { name: /Rate \d stars/ })).toHaveCount(0);
    const downloaded = page.waitForEvent("download");
    await dialog.getByRole("button", { name: "Download" }).click();
    const download = await downloaded;
    const destination = path.join(root, `artifacts/aws-demo-account-download-${name}.md`);
